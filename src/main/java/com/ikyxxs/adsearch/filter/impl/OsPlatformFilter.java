...
@Component
public class OsPlatformFilter implements AdvertFilter {

    @Override
    public boolean validate(SearchParam param) {
        return StringUtils.isNotBlank(param.getOs());
    }

    @Override
    public void filter(BooleanQuery.Builder builder, SearchParam param) {
        Query query = new TermQuery(new Term("os", param.getOs()));
        builder.add(query, BooleanClause.Occur.FILTER);
    }

    @Override
    public FilterType getType() {
        return FilterType.OS_PLATFORM;
    }
